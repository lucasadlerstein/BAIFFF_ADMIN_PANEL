...
const ProductCard = props => {
  ...
  const eliminarTaller = (id) => {
    ConfirmacionSwal.fire({
      ...
      if(result.value){
        try {
          const resEliminar = await clienteAxios.delete(`/api/talleres/${id}`);  
          ConfirmacionSwal.fire({
   ...
   const cambiarVisibilidad = async (id, vi) => {
      ...
      const respuesta = await clienteAxios.put(`/api/talleres/visibilidad/${id}/${viNuevo}`);
      ...
      <CardContent>
        <div className={classes.imageContainer}>
          <img
            ...
            src={`http://api.baifff.tv/static/${product.imagen_es}`}
          />
        </div>
        <Typography
          align="center"
      ...
      <CardActions>
        ...
        >
          ...
          >
            ...
            <a target="_blank" href={`http://baifff.tv/${product.slug_es}`}>
              <Typography
                display="inline"
                variant="body2"
              >
